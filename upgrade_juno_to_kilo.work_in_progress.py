...
class PopulateIDs(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    def validate(self, response, resource, id):
        if response[0] not in REST_SUCCESS_CODES:
            ...
            LOG.error("CMS ID upgrade for resource %(res)s "
                      "id %(id)s failed with error: %(err)s"
                      % {'res': resource,
                         'id': id,
                         'err': err_msg })
        else:
            LOG.debug("CMS ID upgrade for resource %(res)s"
                      "id %(id)s is successful" % {'res': resource,
                                                   'id': id})

    def upgrade_for_cms_id(self):
        cmd_id = self.obtain_cms_id() # TODO wim to implement

        self.handle_subnets()
        self.handle_routers()
        self.handle_ext_networks()
        self.handle_fips()
        self.handle_routes(cms_id)
        self.handle_secgroups()
        self.handle_secgrouprules()
        self.handle_vm_ports()

    def handle_routers(self, cms_id):
    ...
    def handle_subnets(self, cms_id):
            ...
            if not subnet['nuage_managed_subnet']:
                try:
                    ...
                    if subnet['nuage_l2dom_tmplt_id']:
                        url_str = ("cms" + cms_id + "/l2domains/" + subnet['nuage_subnet_id'] +
                                  "?responseChoice=1")
                    else:
                        url_str = ("cms" + cms_id + "/subnets/" + subnet['nuage_subnet_id']
                                  + "?responseChoice=1")
    ...
    def handle_ext_networks(self, cms_id):
    ...
    def handle_secgroups(self, cms_id):
    ...
    def handle_secgrouprules(self, cms_id):
        query = self.context.session.query(
        ...
        for secrule in secgrouprules:
            try:
                ...
                if sgrule['direction'] == 'egress':
                    url_str = "cms" + cms_id + "/ingressaclentrytemplates/"+secrule['nuage_acl_id']\
                              + "?responseChoice=1"
                else:
                    url_str = "cms" + cms_id + "/egressaclentrytemplates/"+secrule['nuage_acl_id']\
                              + "?responseChoice=1"
    ...
    def handle_vm_ports(self, cms_id):
...
def main():
    ...
    parser.add_argument("--setcmsid", action='store_true',
                        help='Option to upgrade for CMS ID, should be used on kilo')
    ...
    if set_cmsID:
        try:
            PopulateIDs(nuageclient).upgrade_for_cms_id()
